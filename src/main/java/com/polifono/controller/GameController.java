...
public class GameController extends BaseController {
	...
	public final String listPhasesOfTheMap(
		...
		if (levelOrder <= 0 || levelOrder > 5) return REDIRECT_HOME;
		...
		if (phases == null) return REDIRECT_HOME;
		
		if (!mapService.playerCanAccessThisMap(map, this.currentAuthenticatedUser().getUser())) return REDIRECT_HOME;
	...
	public final String initPhase(
		...
		if (!playerService.playerHasCredits(this.currentAuthenticatedUser().getUser(), phase) && phase.getOrder() > 1) {
			...
			Phase lastPhaseDone = phaseService.findLastPhaseDoneByPlayerAndGame(this.currentAuthenticatedUser().getUser().getId(), phase.getMap().getGame().getId());
